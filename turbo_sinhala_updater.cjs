...
async function run() {
  console.log('=== Turbo Parallel Sinhala Name Updater Starting ===');
  ...
  console.log(`Extracted ${memberUpdates.length} member names from Excel files.`);

  // High-speed parallel chunk processing (10 workers)
  const concurrency = 10;
  let index = 0;
  let totalDone = 0;

  async function worker() {
    while (index < memberUpdates.length) {
      const i = index;
      index += 50;
      const chunk = memberUpdates.slice(i, i + 50);
      ...
      const pct = Math.round((totalDone / memberUpdates.length) * 100);
      console.log(`Progress: ${totalDone} / ${memberUpdates.length} (${pct}% complete)`);
    }
  }
  ...
  console.log(`🎉 ALL ${totalDone} MEMBER NAMES UPDATED TO SINHALA UNICODE IN HIGH SPEED!`);
}

...
async function run() {
  console.log('=== Starting Complete Member Name Fix from Excel Files ===');
  ...
  console.log(`Extracted ${memberUpdates.length} clean Sinhala member names.`);

  let updatedCount = 0;
  for (let i = 0; i < memberUpdates.length; i += 100) {
    const chunk = memberUpdates.slice(i, i + 100);
    ...
    const pct = Math.round(((i + chunk.length) / memberUpdates.length) * 100);
    console.log(`Updated ${i + 1} to ${i + chunk.length} (${pct}% complete)...`);
  }

  console.log(`🎉 ALL MEMBER NAMES FULLY RESTORED TO SINHALA UNICODE! Total: ${updatedCount}`);
}
